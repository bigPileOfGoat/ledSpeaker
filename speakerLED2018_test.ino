...
byte pattern;                             // randomise starting visualisation
byte animSelect = 0;                      // current manualy selected visualisation, 0 if randomised
int animTimer;                            // sets visualisation duration timer
byte rotatePattern = 0;                   // used for changing or freezing ring rotation (1 in 3 chance of left, right or stop)
...
void setup() {
  ...
  animTimer = random(2000, maxAnimTime);
...
void loop() {
  ...
  if ((currentMillis2 - prevMillis2 > 200) && (ledsOn)) {
    ...
    if ((rotatePattern >= 99) && (rotatePattern < 200)) {
      //rotate 1 to the left
      ringOffset(lRing, 1, true);
      ringOffset(rRing, 1, true);
    }
    if ((rotatePattern >= 199 ) && (rotatePattern < 300)) {
